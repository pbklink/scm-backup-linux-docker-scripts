...
* **build-container.ps1**: Builds a Docker Image containing SCM Backup
...
1. Place the `build-container.ps1` and `start-backup.ps1` scripts into this backup folder.
...
1. Run the `build-container.ps1` script. This will generate the Docker image containing SCM Backup.  It only needs to be run once (unless the image is deleted).
