...
This repository contains 4 scripts which assist with creating a Docker image and then running that image:

* **build-image.sh**: Builds a Docker Image containing SCM Backup
* **start-backup.sh**: Creates the destination folder for the backup and runs the Docker Image to perform the backup.
* and the `PowerShell` equivalent of these scripts (with `.ps1` extension instead).
...
1. If you wish to use the PowerShell scripts, ensure that PowerShell is installed.
...
1. Place either the bash scripts (`build-image.sh` and `start-backup.sh`) or PowerShell scripts (`build-image.ps1` and `start-backup.ps1`) into this backup folder.
...
1. Run the `build-image.sh` or `build-image.ps1` script. This will generate the Docker image containing SCM Backup.  It only needs to be run once (unless the image is deleted).
1. Run the `start-backup.sh` or `start-backup.ps1` script. It will create a sub folder with a name reflecting the current date and time and then start SCM Backup which will carry out a backup according to settings in `settings.yml` and place the backup files in the created sub folder.
